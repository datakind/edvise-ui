...
const TeamManagement = ({ team, availableRoles, userPermissions }) => {
  ...
  const updateRole = () => {
    ...
    updateRoleForm.put(
      route('team-members.update', { team: team.id, user: managingRoleFor.id }),
      {
        preserveScroll: true,
        onSuccess: () => {
  ...
  const removeTeamMember = () => {
    ...
    removeTeamMemberForm.delete(
      route('team-members.destroy', {
        team: team.id,
        user: teamMemberBeingRemoved.id,
      }),
      {
        preserveScroll: true,
  ...
  return (
    <div>
      ...
      {userPermissions.canLeaveTeam && (
        <DangerButton onClick={confirmLeavingTeam} className="mt-3">
          Leave Team
        </DangerButton>
      )}
      ...
      {confirmingLeavingTeam && (
        <FormSection>
          ...
          <div className="flex justify-end">
            <DangerButton onClick={leaveTeam} className="ml-3">
              Leave
            </DangerButton>
      ...
      {teamMemberBeingRemoved && (
        <FormSection>
          <div className="mb-4">
            Are you sure you want to remove {teamMemberBeingRemoved?.name || ''}{' '}
            from the team?
          </div>
          <div className="flex justify-end">
            <DangerButton onClick={removeTeamMember} className="ml-3">
              Remove
            </DangerButton>
