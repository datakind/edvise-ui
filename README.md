...
**Schema contract:** [edvise-api `docs/DB_SCHEMA_CONTRACT.md`](https://github.com/datakind/edvise-api/blob/develop/docs/DB_SCHEMA_CONTRACT.md) defines canonical columns and migration ownership for shared tables. Staging `all_tables` DDL was verified 2026-06-24 (evidence: workspace `github/docs/dbtables/`).

### Shared tables (same physical MySQL database as edvise-api)
...
2. Run UI migrations (`php artisan migrate`).

If you change the `users` table here, update the API ORM and the schema contract in the same or paired PR.
