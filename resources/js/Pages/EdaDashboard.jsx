    ...
    series: [
      {
        ...
        data: items.map(d => ({
          ...
          name: d.name,
          percentage:
            d.percentage > 1 ? Math.round(d.percentage) : d.percentage,
        })),
      },
    ],
  };
...
const createEnrollmentTypeStackedBarOption = ({
  xAxisName,
  categories,
  data,
  legendData,
...
const createVerticalStackedBarOption = ({
  yAxisName,
  xAxisName,
  categories,
  data,
...
const pellRecipientStatus = data => {
  ...
  const derivedCategories =
    data.categories || Object.keys(data.series[0]?.data || {}).sort();
  ...
  return {
    ...
    xAxis: {
      ...
      axisLabel: {
        ...
        interval: 0,
        formatter: value =>
          wrap(String(value ?? ''), { width: 15, trim: true }),
      },
    },
    legend: {
      ...baseOption.legend,
...
export default function EdaDashboard({ batch_id: propBatchId }) {
      ...
      try {
        ...
        if (!batch) {
          if (propBatchId) {
            ...
            throw new Error(
              'No completed batches found. Please create and complete a batch first.',
            );
          }
        }
  ...
  const enrollmentTypeByIntensityOption = edaData?.enrollment_type_by_intensity
    ? createEnrollmentTypeByIntensityOptionFromData(
  ...
  const summaryStats = [
    'total_students',
    'transfer_students',
    'avg_year1_gpa_all_students',
  ].filter(k => edaData?.[k] != null);
  if (!inst_id) {
    return null;
  }
  if (error) {
            ...
            <div className="py-8 text-center text-red-600">
              ...
              {!batchInfo && (
                <p className="mt-4 text-sm">
                  Unable to determine which batch to use. Please provide a
                  batch_id in the URL query parameters or ensure you have at
                  least one completed batch.
                </p>
              )}
              {batchInfo && (
                <p className="mt-4 text-sm">
                  Unable to load data from the backend API. Please check that
                  the batch exists and contains valid data.
                </p>
              )}
            </div>
          </div>
  ...
  return (
    <AppLayout title="EDA Dashboard">
      <Head title="EDA Dashboard" />
      <div
        className={`font-[Helvetica Neue] mb-8 min-w-full ${batchLoading || loading ? 'waiting' : ''}`}
      >
        ...
        <div className="min-w-full bg-[#FAFAFA] p-6">
          ...
          {summaryStats.length > 0 && (
            <div className="mb-5 grid gap-4 md:grid-cols-3">
              {summaryStats.map(k => (
                <StatCard
                  key={k}
                  label={edaData[k].name}
                  value={edaData[k].value}
                />
              ))}
            </div>
          )}
          {edaData?.gpa_by_enrollment_type && (
            <Card
              className="mb-6"
              titleLarge="Average Year 1 GPA by Enrollment Types"
            >
              <div className="grid grid-cols-12 gap-6">
                <div className="col-span-12 md:col-span-4">
                  <p className="mt-4 text-sm font-light text-[#4F4F4F]">
                    This chart shows the average first-year GPA for first-time
                    and transfer students separately. It highlights differences
                    in academic performance by enrollment type, helping you:
                  </p>
                  <ul className="mt-4 list-disc space-y-2 pl-5 text-sm font-light text-[#4F4F4F]">
                    ...
                    <li>
                      Decide whether separate predictive models are needed for
          ...
          {edaData?.gpa_by_enrollment_intensity && (
            <Card
              className="mb-6"
              titleLarge="Average Year 1 GPA by Enrollment Intensity"
            >
              <div className="grid grid-cols-12 gap-6">
                <div className="col-span-12 md:col-span-4">
                  <p className="mt-4 text-sm font-light text-[#4F4F4F]">
                    This chart shows the average first-year GPA for full-time
                    and part-time students. It highlights differences in
                    academic performance by enrollment intensity, helping you:
                  </p>
                  <ul className="mt-4 list-disc space-y-2 pl-5 text-sm font-light text-[#4F4F4F]">
          ...
          {(edaData?.students_by_cohort_term ||
            ...
            <>
              ...
              <div className="mb-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
                {edaData?.students_by_cohort_term && (
                  <Card
                    title="Students by Cohort Year and Term"
                    description="A year by year analysis of when students started their journey at the institution"
                  >
                    {edaData?.students_by_cohort_term && (
                      <EChart
                ...
                {edaData?.course_enrollments && (
                  <Card
                    ...
                    description="Displays total student course enrollments per academic year and semester, showing trends in overall enrollment activity."
                  >
                    {edaData?.course_enrollments && (
                      <EChart
                        option={createHorizontalStackedBarOption({
              ...
              <div className="mb-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
                {edaData?.degree_types?.degrees && (
                  <Card
                    title="Most Common Degree Types"
                    description="The following is a breakdown of degree types being sought by the student body."
                  >
                ...
                {edaData?.enrollment_type_by_intensity && (
                  <Card
                    title="Student Enrollment Type by Intensity"
                    description="This chart signifies students who are first time, re-admitted, or transferred into school, broken down by whether they are enrolled at full time or part time intensity."
                  >
          ...
          {(edaData?.pell_recipient_by_first_gen ||
            ...
            <>
              ...
              <div className="mb-6 text-sm font-light text-[#4F4F4F]">
                DataKind does not use any demographic categories to train our
                model; we only use them to assess bias in the model predictions.
                We display the full demographics of the raw dataset here to make
                sure they match your understanding of your student body. If this
                data looks inaccurate, please confirm you uploaded the correct
                files. If you have further questions, please contact your
              ...
              <div className="mb-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
                ...
                {edaData?.student_age_by_gender && (
                  <Card
                    title="Student Age by Gender"
                    description="An overview of female-identifying vs male-identifying students. This chart is broken down by gender and then by age categories."
                  >
              ...
              {edaData?.race_by_pell_status && (
                <Card
                  className="mb-6"
                  titleLarge="Race by Pell Status"
                  description="This chart shows what race students identify as and who are receiving a Pell Grant."
                >
                  <EChart
                    option={raceByPellStatusOptions(
                      edaData.race_by_pell_status,
                    )}
