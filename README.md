...
Note that the `users` table is the only one that, in production, is shared by the frontend and backend. That means, the table definition needs to be kept in sync between the API repo and this repo. If you make changes to the database here, make sure to make those changes in the `edvise-api` repo as well.
