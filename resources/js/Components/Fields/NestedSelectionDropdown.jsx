...
const NestedSelectionDropdown = ({
  ...
  const transformVariables = variables => {
    ...
    return Object.entries(sectionsMap).map(([name, variables]) => ({
      name,
      variables,
    }));
  };
  ...
  const handleSelect = (sectionName, variable, isOpen) => {
    setSelectedVariables(prevVariables => {
      const availableKeys = availableVariables.map(
        availVar => `${availVar.id}:${availVar.name}`,
      );
      const selectedArray = prevVariables
        ? prevVariables.split(',').map(item => item.trim())
        : [];
